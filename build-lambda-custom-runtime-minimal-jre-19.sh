...
#   1. use the latest Amazon Linux 2 image and install an early access version of OpenJDK 19
...
#   4. feeding the jdeps result into jlink, creating a minimal Java 19 JRE which only contains the necessary modules to run this jar
#   5. create the runtime.zip archive, based on the AWS Lambda custom runtime specification
docker build --progress=plain -t lambda-custom-runtime-minimal-jre-19 .

# extract the runtime.zip from the Docker container and store it locally
docker run --rm --entrypoint cat lambda-custom-runtime-minimal-jre-19 runtime.zip > runtime.zip
